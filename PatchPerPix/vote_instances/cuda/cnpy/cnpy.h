...
namespace cnpy {
    ...
    template<typename T> void npy_save(std::string fname, const T* data, const std::vector<std::uint64_t> shape, std::string mode = "w") {
        FILE* fp = NULL;
        std::vector<std::uint64_t> true_data_shape; //if appending, the shape of existing + new data
        ...
        std::uint64_t nels = std::accumulate(shape.begin(),shape.end(),1,std::multiplies<size_t>());
        std::cout << "cnpy writing, shape:";
        for(unsigned int i = 0; i < shape.size(); i++) {
	        std::cout << shape[i] << " ";
        } std::cout << nels << std::endl;

        fseek(fp,0,SEEK_SET);
        fwrite(&header[0],sizeof(char),header.size(),fp);
        fseek(fp,0,SEEK_END);
        size_t bw = fwrite(data,sizeof(T),nels,fp);
        std::cout << "written " << bw  << "/" << nels << " elements" << std::endl;
        if(ferror(fp))
